...
@property(atomic,copy)RCTResponseSenderBlock globalCallback2;
@end
...
-(void)start:(RCTResponseSenderBlock)callback {
  self.globalCallback2 = callback; // Set the callback so I can pass it back to the presenter later from a different method
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:@"operations"]) {
        ...
        if (self.queue.operationCount == 0 && isFinished == NO) {
            ...
            dispatch_async (dispatch_get_main_queue(), ^{
              if ([self.delegate respondsToSelector:@selector(lanScanDidFinishScanningWithStatus: cbPass:)]) {
                  [self.delegate lanScanDidFinishScanningWithStatus:currentStatus cbPass:self.globalCallback2];
                }
            });
        }
    }
